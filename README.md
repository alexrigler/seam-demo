...
This short guide provides a simple demo showcasing how to control smart locks and access codes for property management software (PMS) using the Seam API. Seam greatly simplifies the integration process with each brand of smart locks. Specifically we'll use Seam to issue unique smart lock access codes for each guest. This access code is then communicated to the Airbnb guest for them to enter the house. Importantly, this access code should only become active at the check-in time for the reservation and then become inactive at the check-out time. We will send this access code to the guest 48 hours before check-in time.


1. Create a Seam account at https://console.seam.co (See [quickstart guide](https://docs.seam.co/latest/quickstart))

2. In your sandbox workspace create test devices using sample data and credentials.
...
3. Go to https://cli.seam.co/ and run `seam` to get started with the online CLI or install the Seam SDK in your preferred language.
...
* PMS server sending this access code to the guest via email & sms 48 hours prior to the check-in
...
In the console you can see the events for a given workspace. Events for access_codes are as follows:
...
- The documentation is thorough and clear to follow. More video content could be useful.

- TODO Intercom onboarding emails. Formatting of code, links to specific API docs pages and demos, include video and image demos e.g. asciinema, carbon
- TODO some example repos + blog posts providing a lightweight PMS demo. Twilio, Stripe, Plaid offers some good examples:
